...
/* Copy, */
pub struct BinaryOp(
    for<'s> fn(Value<'s>, Value<'s>, &mut Stack) -> RuntimeResult<'s, Value<'s>>,
    String,
);
...
impl FromStr for BinaryOp {
    ...
    fn from_str(s: &str) -> Result<Self, ()> {
        let op: for<'s> fn(Value<'s>, Value<'s>, &mut Stack) -> RuntimeResult<'s, Value<'s>> =
            match s {
                "+" => |a, b, _| Value::add(a, b),
                "-" => |a, b, _| Value::sub(a, b),
                "*" => |a, b, s| match (a, b) {
                ...
                ">" => |a, b, _| match (a, b) {
                    ...
                    (Value::Str(mut s), Value::Integer(i)) => match i {
                        0 => Ok(Value::Str(Cow::Owned(String::new()))),
                        i => {
